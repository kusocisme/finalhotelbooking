	...
	<div class="container">
		...
		<ul id="charactersList">
			<c:forEach var="item" items="${sessionScope.wishlist}">
				<li class="character"><a href="hotel?id=${item.hotel.hotelId}">
						<img src="${item.hotelImage}" alt="">
						<div
							style="display: flex; justify-content: space-between; align-items: center;">
							<div>
								<h2>${item.hotel.hotelName}</h2>
								<p>${item.hotel.hotelLocation}</p>
							</div>
							<a href="wishlist?action=remove&hotelId=${item.hotel.hotelId}"
								class="btn"><i style="font-size: 30px;" class="fas fa-heart"></i></a>
						</div>
				</a></li>
			</c:forEach>
		</ul>
